...
function wp_storage_print_image(){
	...
	// 请求的是相应图片的缩略图，缩略图缓存效果一般，有需要的可以打开
	if(isset($_GET['thumbnail']) && $_GET['thumbnail'] == 'true') {
		/*$file_local_path = trailing_slash_path(WP2PCS_TMP_DIR).str_replace('/','_',$image_path).'.thumbnail';
		// 如果存在缓存文件，使用它
		...
		// 如果不存在缓存文件，就从PCS获取，并本地化
			...
			global $baidupcs;
			$result = $baidupcs->thumbnail($image_path, 120, 96);
		...
		$file_local_path = trailing_slash_path(WP2PCS_TMP_DIR).str_replace('/','_',$image_path).'.tmp';
