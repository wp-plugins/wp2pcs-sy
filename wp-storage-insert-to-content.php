		...
		if($('div.selected').length > 0){
			...
			$('div.selected').each(function(){
				...
				// 如果被选择的是视频，使用视频播放器【1.3.0后暂停使用】
					...
					$html += '[video src="'+decodeURIComponent($video_src)+'.m3u8" cover="" <?php echo wp_storage_to_pcs_get_video_size();?> stretch="uniform" refresh="false"]'; //stretch="bestfit"
				}
...
<div id="opt-on-pcs-tabs">
	...
	<p id="manage-buttons">
		...
		<a href="javascript:void(0)" onclick="jQuery('html,body').animate({scrollTop:0},500)" class="button right">回顶部</a>
		<a href="javascript:void(0)" onclick="jQuery('#show-media-alert').toggle();jQuery('html,body').animate({scrollTop:jQuery('#show-media-alert').offset().top},500);" class="button right">提示帮助</a>
	</p>
	<div class="clear"></div>
</div>
<div id="files-on-pcs">
...
function wp_storage_to_pcs_get_video_size() {
	$video_size = get_option("wp_storage_to_pcs_video_size");
	switch ($video_size) {
		case 'MP4_480P':
			$vsize = 'width="640" height="480"';
			break;
		case 'M3U8_854_480':
			$vsize = 'width="854" height="480"';
			break;
		case 'M3U8_640_480':
			$vsize = 'width="640" height="480"';
			break;
		case 'M3U8_480_360':
			$vsize = 'width="480" height="360"';
			break;
		case 'M3U8_480_224':
			$vsize = 'width="480" height="224"';
			break;
		case 'M3U8_320_240':
