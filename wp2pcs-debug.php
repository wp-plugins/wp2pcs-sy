 ...
 */
	...
	echo "你当前使用的是个人标准版 [保存在自己的网盘] 版本号：$plugin_version 最后更新时间：$version <br />";
	...
	if(!$baidupcs || !$quota || isset($quota->error_code)){
		if(get_option('wp_to_pcs_access_token')){
			echo '<p style="color:red;"><b>连接失败，有可能你的网站服务器和百度PCS通信不良！</b></p>';
		}else{
			echo '<p style="color:red;"><b>授权失败，无法连接到百度网盘，点击“更新授权”再授权！</b></p>';
		}
	}else{
		echo '百度PCS授权成功 Access Token:'.get_option('wp_to_pcs_site_id').' ';
		echo '<p>当前网盘总'.number_format(($quota->quota/(1024*1024)),2).'MB，剩余'.number_format((($quota->quota - $quota->used)/(1024*1024*1024)),2).'GB。</p>';
	}
	...
	if($outlink_type == 200){
		...
		$outlink_type = "外链";
	} ，附件访问方式为： $outlink_type
	*/
	echo "7.图片最终路径为 $image_path <br />";
